...
class QQMusicResource:
    ...
    def get_music_url(self, songmid, file_type="128"):
        ...
        if file_type not in self.file_config:
            raise ValueError(
                "Invalid file_type. Choose from 'm4a', '128', '320', 'flac'"
            )
        ...
        response = requests.post(
            self.base_url, json=req_data, cookies=self.cookies, headers=self.headers
        )
        data = response.json()
        # print(data)
        ...
        bitrate = next(
            (
                info["bitrate"]
                for key, info in self.file_config.items()
                if info["s"] == prefix
            ),
            "",
        )
