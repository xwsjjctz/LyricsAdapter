...
截至当前分支 `refactor-frontend-style-structure`，本计划的主线能力已完成：

- 实验性新 UI 开关、样式文件分层、`NewUxShell` 分流已落地，旧 UI 默认路径保留。
- MainView 已展示 local/cloud/online 歌单卡片，使用 APP Icon 采样后的固定高斯模糊背景作为新 UI 默认根背景。
- 歌曲列表、元数据编辑、删除确认已通过同层浮动面板组织，同一时刻只保留一个歌曲列表面板。
- 歌单卡片和歌曲条目的右键菜单已迁移到新 UI，编辑模式菜单保持独立分支。
- 独立定位按钮、浮动播放器面板、mini player 子组件拆分已完成。
- FocusMode 已保留 legacy 默认样式，并通过可选 `ambientLayer` / `variant` 接入新 UI 流光背景和进入过渡。
- FocusMode 已拆分出背景层、封面区、曲目信息区和播放控制条；mini player 到 FocusMode 的过渡已覆盖 cover/title/artist/album/progress/controls。

明确延后、不阻塞本轮计划收尾的项：

- `刷新云端歌曲` 菜单已预留入口，但真实 WebDAV 刷新流程后续单独接入。
- `清空在线播放历史` 已预留入口，历史管理能力后续补齐。
- 用户自定义歌单的重命名、删除等管理入口后续在歌单数据模型稳定后接入。
- `下一首播放` 属于队列能力扩展，本轮不实现。
